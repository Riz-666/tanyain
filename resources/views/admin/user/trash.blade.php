                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="user-table flex items-center gap-3">
                                            @if ($item->foto)
                                                <img src="{{ asset('storage/profile-trash/' . $item->foto) }}" alt="Avatar"
                                    ...
                                    <td>
                                        <div class="action-buttons">
                                            <form action="{{ route('admin.user.restore', $item->id) }}" method="post"
                                                ...
                                                @csrf
                                                <button data-id-alert-restore-="{{ $item->id }}"
                                                    class="btn btn-restore"><i
                                                        class="fas fa-undo me-1"></i>Pulihkan</button>
                                            </form>
                                            <form action="{{ route('admin.user.forceDelete', $item->id) }}" method="post"
                                                class="d-inline" id="form-delete-alert-{{ $item->id }}">
                                                @csrf
                                                <button class="btn btn-delete-permanent"
                                                    data-id-alert-="{{ $item->id }}"><i
