                        ...
                        <tbody>
                            @foreach ($data as $item)
                                <tr>
                                    ...
                                    <td>
                                        <div class="user-table flex items-center gap-3">
                                            ...
                                            <div class="user-details">
                                                ...
                                                <br>
                                                <span class="text-xs opacity-90">ARTIKEL ID : {{ $item->id }}</span>
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                                        @if($item->repositoriSoftDelete)
                                            <span class="badge bg-info text-dark">{{ str::limit($item->repositoriSoftDelete->judul_repo,20) }}</span>
                                        @else
                                            <span class="badge bg-info text-dark">Tidak Ada Repositori Terkait</span>
                                        @endif
                                    </td>
                                    <td>
                                        <div class="user-table flex items-center gap-3">
                                    ...
                                    <td><span><i class="fa fa-eye"></i> {{ $item->views ?? 0 }}</span></td>
                                    <td><span class="badge bg-secondary">{{ $item->deleted_at }}</span></td>
                                    <td>
                                        <div class="action-buttons">
                                            <form action="{{ route('artikel.trash.restore', $item->id) }}" method="post"
                                            ...
                                            <form action="{{ route('artikel.trash.forceDelete', $item->id) }}" method="post"
                                                class="d-inline" id="form-delete-alert-{{ $item->id }}">
                                                @csrf
                                                <button type="submit" class="btn btn-delete-permanent" data-id-alert-="{{ $item->id }}"><i
